...
from app.mcp_client import list_labs_tools, list_tour_tools
...
async def mcp_tools() -> dict:
    """연결된 두 MCP 서버의 도구 목록을 확인합니다."""
    try:
        return {
            "labs": await list_labs_tools(),
            "tour": await list_tour_tools(),
        }
    except Exception as error:
        raise HTTPException(
            status_code=503,
            detail=f"MCP 서버 연결 실패: {error}",
        ) from error